...
/**
 * DEPRECATED: This report is deprecated and replaced by the new report 
 */

export function runCopilotAssociationsReport(): string | undefined {
  ...
  const json_data = getAllOrgCopilotAssociations({ orgs: org_data });
  const csv_file = writeToCsv(json_data, "copilot_associations.csv");
  
  return csv_file;
}

function getAllOrgCopilotAssociations({
  orgs,
}: {
  ...
  for(const org_name in orgs) {
    ...
    const associations = getCopilotAssociations({ org_data });
    results.push(...associations);
  }
  return results;
}

function getCopilotAssociations({
  org_data,
}: { 
  org_data: ActivityData; 
}): CopilotAssociation[] { 
  const team_associations = getTeamAssociations({ org_data });
  const repository_associations = getRepositoryAssociations({ org_data }); 
  return team_associations.concat(repository_associations);
}

function getTeamAssociations({
  org_data,
}: { 
  org_data: ActivityData; 
}): CopilotAssociation[] {
...
function getRepositoryAssociations({
  org_data,
}: { 
  org_data: ActivityData; 
}) {
